...
報告中的零件圖採用**精美 SVG 向量示意圖**，並以**精確型號標註**（CPU / 主機板 / 記憶體 / 顯卡 / SSD / 水冷 / 機殼 / 電源）。

> ⚠️ 原本計畫抓取官方真實產品照，但此環境封鎖了主要圖庫來源（Gigabyte / DuckDuckGo 回 403、Bing 無 JS 退化、Wikimedia 400）。為避免放錯產品或破圖（即「搞混」），改用自製向量圖。
> 👉 若日後要換成真實照片，只要把圖檔放進 [`assets/images/`](assets/images/) 並以零件命名即可無痛替換。
